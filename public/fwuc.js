...
(function() {
    ...
    function showAuthModal(appid, onValid){
        if (document.querySelector('.fwuc-auth-overlay')) return;
        const overlay = document.createElement('div');
        overlay.className = 'fwuc-auth-overlay';
        ...
        const modal = document.createElement('div');
        modal.style.cssText = 'background:#1b2838;color:#fff;border:1px solid #2a475e;border-radius:4px;min-width:360px;max-width:560px;padding:18px 20px;box-shadow:0 8px 24px rgba(0,0,0,.6);';
        const title = document.createElement('div');
        title.style.cssText = 'font-size:16px;color:#66c0f4;margin-bottom:10px;font-weight:600;';
        title.textContent = 'fwuc · Authentication';
        const info = document.createElement('div');
        info.style.cssText = 'font-size:13px;color:#8f98a0;margin-bottom:8px;';
        info.textContent = 'Enter your key from Discord bot';
        const input = document.createElement('input');
        input.type = 'text'; input.placeholder = 'Key';
        input.style.cssText = 'width:100%;padding:8px 10px;border:1px solid #2a475e;background:#101822;color:#fff;border-radius:3px;outline:none;';
        const error = document.createElement('div');
        error.style.cssText = 'margin-top:8px;color:#ff6666;font-size:12px;display:none;';
        const btnRow = document.createElement('div');
        btnRow.style.cssText = 'margin-top:12px;display:flex;gap:8px;justify-content:flex-end;';
        const cancel = document.createElement('a');
        cancel.className = 'btnv6_blue_hoverfade btn_medium';
        cancel.innerHTML = '<span>Cancel</span>';
        cancel.href = '#';
        cancel.onclick = function(e){ e.preventDefault(); overlay.remove(); };
        const submit = document.createElement('a');
        submit.className = 'btnv6_blue_hoverfade btn_medium';
        submit.innerHTML = '<span>Continue</span>';
        submit.href = '#';
        submit.onclick = function(e){
            e.preventDefault();
            const key = String(input.value || '').trim();
            if (!key){ error.textContent='Enter key'; error.style.display='block'; return; }
            try{
                Millennium.callServerMethod('fwuc','AuthValidateKey',{ key, contentScriptQuery:'' }).then(function(res){
                    try{
                        const payload = typeof res==='string'? JSON.parse(res): res;
                        if (payload && payload.success && payload.valid){
                            try { localStorage.setItem('fwucSavedKey','1'); } catch(_){ }
                            overlay.remove(); onValid();
                        }
                        else { error.textContent = 'Invalid key'; error.style.display='block'; }
                    }catch(_){ error.textContent='Error'; error.style.display='block'; }
                });
            }catch(_){ error.textContent='Error'; error.style.display='block'; }
        };
        btnRow.appendChild(cancel); btnRow.appendChild(submit);
        modal.appendChild(title); modal.appendChild(info); modal.appendChild(input); modal.appendChild(error); modal.appendChild(btnRow);
        overlay.appendChild(modal);
    ...
    function addfwucButton() {
        ...
        if (steamdbContainer) {
            ...
            fwucButton.setAttribute('data-fwuc-mode', 'add');
            // Normalize margins to match native buttons
            try {
                if (referenceBtn) {
                    const cs = window.getComputedStyle(referenceBtn);
                    ...
                    Millennium.callServerMethod('fwuc', 'HasfwucForApp', { appid, contentScriptQuery: '' }).then(function(res){
                        try {
                            ...
                            if (payload && payload.success && payload.exists === true) {
                                ...
                                const btn = fwucButton.cloneNode(true);
                                btn.setAttribute('data-fwuc-mode', 'remove');
                                const s = btn.querySelector('span');
                                if (s) s.textContent = 'Remove via fwuc';
                                btn.title = 'Remove via fwuc';
                                btn.setAttribute('data-tooltip-text', 'Remove via fwuc');
                                if (!document.querySelector('.fwuc-button') && !window.__fwucButtonInserted) {
                                    ...
                                    backendLog('fwuc remove button inserted');
                                }
        ...
        if (anchor) {
            ...
            try {
                ...
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    ...
                    const mode = anchor.getAttribute('data-fwuc-mode') || 'add';
                    if (mode === 'remove') {
                        runState.inProgress = true; runState.appid = appid;
                        Millennium.callServerMethod('fwuc', 'RemovefwucForApp', { appid, contentScriptQuery: '' }).then(function(res){
                            try{
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                if (payload && payload.success) {
                                    anchor.setAttribute('data-fwuc-mode', 'add');
                                    const s = anchor.querySelector('span');
                                    if (s) s.textContent = 'Add via fwuc';
                                    anchor.title = 'Add via fwuc';
                                    anchor.setAttribute('data-tooltip-text', 'Add via fwuc');
                                }
                            }catch(_){ }
                            runState.inProgress = false; runState.appid = null;
                            const overlay = document.querySelector('.fwuc-overlay'); if (overlay) overlay.remove();
                        });
                    } else {
                        const proceed = function(){
                            runState.inProgress = true; runState.appid = appid;
                            if (!document.querySelector('.fwuc-overlay')) { showTestPopup(); }
                            Millennium.callServerMethod('fwuc', 'StartAddViafwuc', { appid, contentScriptQuery: '' });
                            startPolling(appid);
                        };
                        
                        const validateKey = function(key, source) {
                            if (!key) return false;
                            return Millennium.callServerMethod('fwuc', 'AuthValidateKey', { key: key, contentScriptQuery: '' }).then(function(res){
                                try {
                                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                if (payload && payload.success && payload.valid) {
                                    if (source === 'localStorage') {
                                        localStorage.setItem('fwucSavedKey', '1');
                                        localStorage.setItem('fwuc_auth_key', key);
                                    ...
                                    return true;
                                        ...
                                        if (source === 'localStorage') {
                                            localStorage.removeItem('fwucSavedKey');
                                            localStorage.removeItem('fwuc_auth_key');
                                        }
                                        showAuthModal(appid, proceed);
                                        return false;
                                    }
                                } catch(_) {
                                    if (source === 'localStorage') {
                        ...
                        const lsSaved = (function(){ try { return localStorage.getItem('fwucSavedKey')==='1'; } catch(_){ return false; } })();
                        if (lsSaved) { 
                            const savedKey = (function(){ try { return localStorage.getItem('fwuc_auth_key'); } catch(_){ return null; } })();
                            if (savedKey) {
                                validateKey(savedKey, 'localStorage');
                            } else {
                                localStorage.removeItem('fwucSavedKey');
                                showAuthModal(appid, proceed);
                            }
                        ...
                        const checkRegistry = function() {
                            Millennium.callServerMethod('fwuc','HasSavedKey',{}).then(function(r){
                                try{
                                    const p = typeof r==='string'? JSON.parse(r): r;
                                    if (p && p.success && p.saved) {
                                        const registryKey = p.key;
                                        if (registryKey) {
                                            validateKey(registryKey, 'registry');
                                        } else {
                                            showAuthModal(appid, proceed);
                                        }
                                    } else {
                ...
                Millennium.callServerMethod('fwuc', 'GetAddViafwucStatus', { appid, contentScriptQuery: '' }).then(function(res){
                    try {
                        ...
                        if (st.status === 'done'){
                            ...
                            runState.inProgress = false; runState.appid = null;
                            // toggle button to Remove via fwuc
                            const btnEl = document.querySelector('.fwuc-button');
                            if (btnEl) {
                                btnEl.setAttribute('data-fwuc-mode', 'remove');
                                const s = btnEl.querySelector('span');
                                if (s) s.textContent = 'Remove via fwuc';
                                btnEl.title = 'Remove via fwuc';
                                btnEl.setAttribute('data-tooltip-text', 'Remove via fwuc');
                            }
    ...
    function showAuthModal(appid, onSuccess) {
        // Avoid duplicates
        if (document.querySelector('.fwuc-auth-overlay')) return;
        ...
        const modal = document.createElement('div');
        modal.style.cssText = 'background:#1b2838;color:#fff;border:1px solid #2a475e;border-radius:8px;min-width:400px;max-width:500px;padding:24px;box-shadow:0 8px 24px rgba(0,0,0,.8);';

        const title = document.createElement('div');
        title.style.cssText = 'font-size:20px;color:#66c0f4;margin-bottom:16px;font-weight:600;text-align:center;';
        title.textContent = 'FireWatchUnlocker Authentication';

        const subtitle = document.createElement('div');
        subtitle.style.cssText = 'font-size:14px;color:#c7d5e0;margin-bottom:20px;text-align:center;';
        subtitle.textContent = 'Enter your key from the Discord bot';

        const inputGroup = document.createElement('div');
        inputGroup.style.cssText = 'margin-bottom:20px;';

        const input = document.createElement('input');
        input.type = 'text';
        input.placeholder = 'Enter your key here';
        input.style.cssText = 'width:100%;padding:12px;background:#2a475e;border:1px solid #417a9b;border-radius:4px;color:#fff;font-size:14px;box-sizing:border-box;';
        ...
        errorDiv.style.cssText = 'color:#ff6b6b;font-size:12px;margin-top:8px;display:none;';
        errorDiv.className = 'fwuc-auth-error';
        ...
        cancelBtn.style.cssText = 'padding:10px 20px;background:#2a475e;border:1px solid #417a9b;border-radius:4px;color:#c7d5e0;cursor:pointer;font-size:14px;';
        cancelBtn.addEventListener('click', function() {
            document.body.removeChild(overlay);
        });

        const validateBtn = document.createElement('button');
        validateBtn.textContent = 'Authenticate';
        validateBtn.style.cssText = 'padding:10px 20px;background:#417a9b;border:1px solid #66c0f4;border-radius:4px;color:#fff;cursor:pointer;font-size:14px;';
                ...
                Millennium.callServerMethod('fwuc', 'AuthValidateKey', { key, contentScriptQuery: '' }).then(function(res) {
                    try {
                        ...
                        if (payload && payload.success && payload.valid) {
                            ...
                            showError('Invalid key. Please check your key and try again.');
                            validateBtn.textContent = 'Authenticate';
