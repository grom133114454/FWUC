...
import requests
try:
    from . import auth_db  # type: ignore
except Exception:
...
class Plugin:
    ...
    def HasValidSavedKey(self) -> str:
        try:
            ok = auth_db.has_valid_saved_key()
            return json.dumps({ 'success': True, 'valid': bool(ok) })
        except Exception as e:
            return json.dumps({ 'success': False, 'error': str(e) })

    def HasSavedKey(self) -> str:
        try:
            ok = auth_db.has_saved_key()
            return json.dumps({ 'success': True, 'saved': bool(ok) })
        except Exception as e:
            return json.dumps({ 'success': False, 'error': str(e) })

    def auth_validate_key(self, key: str) -> str:
        try:
            ok = auth_db.validate_key(key)
            return json.dumps({ 'success': True, 'valid': bool(ok) })
        except Exception as e:
            return json.dumps({ 'success': False, 'error': str(e) })
...
def detect_steam_install_path() -> str:
            ...
            if winreg is not None:
                with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Valve\Steam") as key:
                    ...
                    if path and os.path.exists(path):
                        logger.log(f'fwuc: Steam path found in registry: {path}')
        except Exception as e:
            logger.warn(f'fwuc: Registry detection failed: {e}')
    ...
    if not path or not os.path.exists(path):
        try:
            ...
            if path and os.path.exists(path):
                logger.log(f'fwuc: Steam path found via Millennium: {path}')
        except Exception as e:
            logger.warn(f'fwuc: Millennium detection failed: {e}')
    ...
    if not path or not os.path.exists(path):
        ...
        for p in common_paths:
            if os.path.exists(p):
                path = p
                logger.log(f'fwuc: Steam path found in common location: {path}')
                break
    ...
    logger.warn('fwuc: Failed to detect Steam install path')
    return ''
...
def RemovefwucForApp(appid: int, contentScriptQuery: str = '') -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({ 'success': False, 'error': 'Invalid appid' })
    ...
    try:
        ...
        logger.error(f'fwuc: Remove failed for {appid}: {e}')
        return json.dumps({ 'success': False, 'error': str(e) })

def AuthValidateKey(key: str, contentScriptQuery: str = '') -> str:
    try:
        ok = auth_db.validate_key(str(key or ''))
        return json.dumps({ 'success': True, 'valid': bool(ok) })
    except Exception as e:
        logger.error(f'fwuc: AuthValidateKey failed: {e}')
        return json.dumps({ 'success': False, 'error': str(e) })
...
def _set_download_state(appid: int, update: dict) -> None:
    with DOWNLOAD_LOCK:
        ...
        try:
            if 'status' in update:
                logger.log(f"fwuc: Download state for {appid} changed to {update['status']}")
        except Exception:
            pass
...
def _download_zip_for_app(appid: int):
    ...
    urls = [
        ...
        f"https://mellyiscoolaf.pythonanywhere.com/m/{appid}",
                ...
                try:
                    if name.lower().endswith('.manifest'):
                        ...
                        with open(out_path, 'wb') as mf:
                            mf.write(data)
                        logger.log(f"fwuc: Extracted manifest -> {out_path}")
                except Exception as me:
                    logger.warn(f"fwuc: Failed to extract manifest {name}: {me}")
        except Exception as e:
            logger.warn(f"fwuc: depotcache extraction failed: {e}")
...
def HasSavedKey(contentScriptQuery: str = '') -> str:
    try:
        import auth_db
        key = auth_db.read_key_registry()
        if key:
            return json.dumps({ 'success': True, 'saved': True, 'key': key })
        else:
            return json.dumps({ 'success': True, 'saved': False })
    except Exception as e:
        logger.error(f'fwuc: HasSavedKey error: {e}')
        return json.dumps({ 'success': False, 'error': str(e) })

def AuthValidateKey(key: str, contentScriptQuery: str = '') -> str:
    try:
        import auth_db
        is_valid = auth_db.validate_key(key)
        return json.dumps({ 'success': True, 'valid': is_valid })
    except Exception as e:
        logger.error(f'fwuc: AuthValidateKey error: {e}')
        return json.dumps({ 'success': False, 'error': str(e) })
